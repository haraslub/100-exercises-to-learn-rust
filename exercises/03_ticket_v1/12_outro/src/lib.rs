...
impl Order {
    pub fn new(product_name: String, quantity: u32, unit_price: u32) -> Order {
        ...
        Self::is_greater_than_zero(quantity);
        Self::is_greater_than_zero(unit_price);
    ...
    pub fn set_quantity(&mut self, new_quantity: u32) {
        Self::is_greater_than_zero(new_quantity);
        self.quantity = new_quantity;
    }

    pub fn set_unit_price(&mut self, new_unit_price: u32) {
        Self::is_greater_than_zero(new_unit_price);
    ...
    fn validate_product_name(product_name: &String) {
        ...
        if product_name.len() > MAX_LENGTH {
            panic!("Cannot be longer than {}", MAX_LENGTH)
        }
    }

    fn is_greater_than_zero(quantity: u32) {
        if quantity <= 0 {
