...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        ...
        Err(TicketNewError::TooLongTitle) => panic!("Title cannot be longer than 50 bytes"),
        Err(TicketNewError::EmptyDescription) => Ticket::new(title, "Description not provided".to_string(), status).unwrap(),
        Err(TicketNewError::TooLongDescription) => Ticket::new(title, "Description not provided".to_string(), status).unwrap(),
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TooLongTitle);
        }
        if description.is_empty() {
            return Err(TicketNewError::EmptyDescription);
        }
        if description.len() > 500 {
            return Err(TicketNewError::TooLongDescription);
        }
